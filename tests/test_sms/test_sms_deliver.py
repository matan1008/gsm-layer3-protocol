...
from gsm_layer3_protocol.sms_protocol.called_party_bcd_address import AddressField
...
def test_building_sms_deliver_with_data_header():
    originator_address = AddressField(number_type.INTERNATIONAL_NUMBER, plan.UNKNOWN, "123456")
    ...
    assert build(L3Message(
        ...
        CpData(rp_mti.RP_DATA_N_TO_MS, RpDataNToMs(1, originator_address, sms_deliver))
    )) == b"\x39\x01\x29\x01\x01\x04\x90\x21\x43\x65\x00\x20\x64\x06\x90\x21\x43\x65\x00\x00\x81\x90\x10\x32\x60\x00\x80\x12\x05\x00\x03\x03\x02\x01\x88\x65\x76\xda\x5e\x96\x83\xea\xf3\x10"
